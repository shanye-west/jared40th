...
type YardageRow = {
  label: string;
  yards: (number | undefined)[];  // 18-element array
};
...
export function ScorecardTableHeader({ holes, totals, yardageRows }: ScorecardTableHeaderProps) {
  ...
  return (
    <thead>
      ...
      {/* Yardage Row(s) */}
      {(yardageRows && yardageRows.length > 0 ? yardageRows : [{ label: "Yards", yards: holes.map(h => h.yards) }]).map((row, rowIdx) => (
        <tr key={rowIdx} className="bg-slate-50 text-slate-900 text-xs border-b border-slate-200">
          <td className="sticky left-0 z-10 bg-slate-50 text-left px-3 py-1">{row.label}</td>
          {row.yards.slice(0, 9).map((y, i) => (
