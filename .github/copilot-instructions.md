...
- `courses` - Hole data: `par`, `hcpIndex` per hole
- `playerMatchFacts` - Immutable per-match stats per player
...
| `twoManShamble` | Same + `teamADrive`, `teamBDrive` | Best GROSS per team (no strokes) |
| `twoManScramble` | `teamAGross`, `teamBGross` + drives | Team gross vs team gross |

**Handicap**: `strokesReceived` is 18-element array of 0/1; max 1 stroke per hole.
...
- Check `isFourPlayerRows` for best ball/shamble (4 player inputs)
- Check `isTeamFormat` for scramble (2 team inputs)
